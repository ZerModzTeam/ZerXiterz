// Pastikan Config Firebase bener sesuai screenshot lo
...
// Login pake data dari Database lo
function auth() {
    const u = document.getElementById('adm-u').value, p = document.getElementById('adm-p').value;
    showStatus('process', 'MENGHUBUNGKAN...');
    db.ref('admin_config').once('value').then(snap => {
        const d = snap.val();
        if(d && u === d.user && p === d.pass) {
            showStatus('success', 'DISETUJUI', () => {
                document.getElementById('admin-panel').style.display = 'block';
                load();
            });
        } else { alert("Akses Ditolak!"); document.getElementById('overlay-action').style.display='none'; }
    });
}

function showStatus(type, msg, callback) {
    document.getElementById('login-form').style.display = 'none';
    const ui = document.getElementById('status-ui'), scs = document.getElementById('l-success'), spin = document.getElementById('spinner'), txt = document.getElementById('l-text');
...
function save() {
    ...
    if(n && d && p) {
        ...
        showStatus('process', 'MENYIMPAN...');
        if(id) {
            db.ref('products/'+id).update(data).then(() => showStatus('success', 'BERHASIL EDIT', () => cancelEdit()));
        } else {
            db.ref('products').push(data).then(() => showStatus('success', 'BERHASIL PUBLISH', () => cancelEdit()));
        }
    }
}
        ...
        snap.forEach(c => {
            ...
            if(i.main === curM && (curS === 'ALL' || i.sub === curS) && i.name.toLowerCase().includes(q)) {
                ...
                list.innerHTML += `<div class="card">
                    ...
                    <p style="color:#aaa;margin-bottom:15px">${i.dur} Day | ${i.price}K</p>
                    <a href="https://wa.me/6289653938936?text=${msg}" target="_blank" class="btn-glow" style="display:block;text-align:center;text-decoration:none">BELI SEKARANG</a>
                </div>`;
            }
            admList.innerHTML += `<div class="adm-item">
                <span onclick="editMode('${k}')" style="color:var(--p);cursor:pointer;font-weight:bold">${i.name}</span>
                <button onclick="db.ref('products/${k}').remove()" style="color:red;background:none;border:none;cursor:pointer">HAPUS</button>
            </div>`;
        });
    });
}
...
let stars = Array(150).fill().map(() => ({ x: Math.random()*cvs.width, y: Math.random()*cvs.height, s: Math.random()*0.4 }));
function draw() {
