...
// DASHBOARD HANYA PUNYA PILIHAN SPESIFIK (ALL DIHAPUS DARI DROPDOWN)
const subDataAdmin = { 
    APK: ['ROOT/VIRTUAL', 'NO ROOT'], 
    FILE: ['HOLO', 'HEADSHOT'] 
};

// MENU UTAMA PUNYA PILIHAN ALL
const subDataDisplay = {
    APK: ['ALL', 'ROOT/VIRTUAL', 'NO ROOT'],
    FILE: ['ALL', 'HOLO', 'HEADSHOT']
};
...
function save() {
    ...
    if(n && d && p) {
        ...
        (id ? ref.update(data) : ref.push(data)).then(() => showStatus('success', 'SUCCESS', () => { cancelEdit(); load(); }));
    }
}
...
function load() {
    db.ref('products').on('value', snap => {
        ...
        snap.forEach(c => {
            ...
            // FILTER LOGIC: KALO PILIH ALL, SEMUA SUB DARI MAIN CAT MUNCUL
            ...
            adm.innerHTML += `<div style="background:#111; padding:12px; border-radius:10px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; border:1px solid #222;">
                <div><b style="color:var(--p)">${i.name}</b><br><small style="color:#555">${i.sub}</small></div>
                <div style="display:flex; gap:15px;"><i class="fas fa-edit" onclick="editMode('${k}')" style="color:orange;"></i><i class="fas fa-trash" onclick="if(confirm('Hapus?')) db.ref('products/${k}').remove()" style="color:red;"></i></div>
            </div>`;
        });
    });
}
...
function renderSub() {
    ...
    // RENDER SUB MENU DI MAIN DISPLAY (ADA PILIHAN 'ALL')
...
function setADM(m) {
    ...
    // DI DASHBOARD ADMIN GAK ADA PILIHAN 'ALL'
    subDataAdmin[m].forEach(s => sel.innerHTML += `<option value="${s}">${s}</option>`);
}
